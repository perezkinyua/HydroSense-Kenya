"""
Contains algorithmic solvers for optimizing irrigation schedules.
Uses a Model Predictive Control (Greedy) approach to minimize water usage 
while strictly adhering to biological crop thresholds.
"""
import numpy as np

def optimize_irrigation_schedule(rain_array, et_array, S_initial, S_min, S_target, S_cap):
    """
    Computes the optimal daily irrigation schedule to minimize water usage.
    ...
    - optimized_moisture: 1D array of resulting soil moisture (%)
    """
    ...
    optimized_moisture = np.zeros(N_days + 1)
    optimized_moisture[0] = S_initial

    for t in range(N_days):
        # 1. Predict tomorrow's moisture without any irrigation
        projected_S = optimized_moisture[t] + rain_array[t] - et_array[t]
        ...
        if projected_S < S_min:
            ...
            optimized_moisture[t+1] = projected_S + water_needed
        else:
            # No irrigation needed
            optimized_irrigation[t] = 0.0
            optimized_moisture[t+1] = projected_S
            
        # 4. Drainage Cap: If massive rain pushes moisture above field capacity, it drains away
        if optimized_moisture[t+1] > S_cap:
            optimized_moisture[t+1] = S_cap
            
    return optimized_irrigation, optimized_moisture